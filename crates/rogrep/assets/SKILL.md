...
- Facets: `provider:` (claude|codex|cursor|grok|hermes|opencode), `model:`, `project:`, `cwd:`, `file:`, `role:`, `tool:` (tool name), `tool_cmd:` (shell executable, e.g. `tool_cmd:git`), `tool_status:` (succeeded|failed|rejected), `tool_mutating:true`, `git_cmd:` (commit|push|…), `git_pr:` (create|merge|…), `git_pr_num:N`, `git_commit:SHA`, `git_branch:`, `is:` (interrupted|compacted|notification|subagent), `origin:` (interactive|subagent|scheduled|auxiliary), `subagent:` (true|false), `since:`/`before:` (YYYY-MM-DD or Nd, e.g. `since:7d`). Facet values accept `*`/`?` wildcards and `/regex/`.
