...
- Query grammar: bare terms AND together; `"quoted phrases"` match in order; `/regex/` patterns match full turn text (case-sensitive; use `(?i)` inside the pattern for case-insensitive, `\s` for whitespace — a pattern cannot contain spaces); `key:value` facets filter. Unknown `key:` tokens (URLs, `data:` URIs) are treated as literal text — only known facet keys activate. Facet values accept `*`/`?` wildcards and anchored `/regex/`.
- Substring facets (case-insensitive, e.g. `model:sonnet` matches `claude-sonnet-…`): `provider:`/`agent:` (claude|codex|cursor|grok|hermes|opencode), `model:`, `project:`, `cwd:`, `file:` (paths touched by tool calls), `source:` (rollout file path).
- Exact-value facets: `role:` (user|assistant|tool|system|event), `origin:` (interactive|subagent|scheduled|auxiliary), `subagent:` (true|false), `is:` (interrupted|compacted|notification|scheduled|subagent), `content:image`, `tool:` (tool name), `skill:`, `mcp:` (server of `mcp__server__tool`), `tool_cmd:` (shell executable, e.g. `tool_cmd:git`), `tool_type:` (tests|build|search|git-operation|git-inspection|http-request|package-management|file-inspection|read-file|edit-file|…), `tool_location:` (local|remote), `tool_mutability:` (read-only|mutating), `tool_privilege:privileged`, `tool_status:` (succeeded|failed|rejected), `tool_mutating:true`, `git_cmd:` (commit|push|…), `git_pr:` (create|merge|…), `git_pr_num:N`, `git_commit:SHA`, `git_branch:`, `git_remote:`. Values normalize `_`→`-` on both sides.
- Date facets (YYYY-MM-DD or Nd, intersect with each other and `--since`): `since:`/`after:` (lower bound, e.g. `since:7d`), `before:`/`until:` (upper bound), `when:` (one day).
- Full reference: docs/query-syntax.md in the rogrep repo.
...
- Corpus search covers user-visible turns of real work: harness-injected context blocks and machine-evaluation sessions (codex auto-review judges, `origin:auxiliary`) are excluded by default. Use `find` to grep context blocks inside one conversation, or an explicit `origin:auxiliary` facet to search evaluation sessions.
