...
- Facets: `provider:` (claude|codex|cursor|grok|hermes|opencode), `model:`, `project:`, `cwd:`, `file:`, `source:` (rollout file path), `role:`, `tool:` (tool name), `tool_cmd:` (shell executable, e.g. `tool_cmd:git`), `tool_type:` (tests|build|search|git-operation|git-inspection|http-request|…), `tool_location:` (local|remote), `tool_mutability:` (read-only|mutating), `tool_privilege:privileged`, `tool_status:` (succeeded|failed|rejected), `tool_mutating:true`, `git_cmd:` (commit|push|…), `git_pr:` (create|merge|…), `git_pr_num:N`, `git_commit:SHA`, `git_branch:`, `is:` (interrupted|compacted|notification|subagent), `origin:` (interactive|subagent|scheduled|auxiliary), `subagent:` (true|false), `since:`/`before:` (YYYY-MM-DD or Nd, e.g. `since:7d`). Metadata facets (`provider:`/`model:`/`project:`/`cwd:`/`file:`/`source:`) match case-insensitive substrings (`model:sonnet` matches `claude-sonnet-…`); the rest are exact values. Facet values accept `*`/`?` wildcards and `/regex/`.
